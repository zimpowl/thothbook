"""Point d'entrée : `python -m le_cadre` (chat) ou `python -m le_cadre --seed`."""
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="le_cadre", description="Le Cadre — organisation personnelle.")
    ...
    if args.seed:
        ...
        console.print("[bold blue]Le Cadre[/] → ouvre http://localhost:8000 dans ton navigateur")
        uvicorn.run("le_cadre.api:app", host="127.0.0.1", port=8000, reload=False)
    else:
        lancer_chat()
